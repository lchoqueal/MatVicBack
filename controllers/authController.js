...
const authController = {
  ...
  async login(req, res) {
    ...
    try {
      ...
      const adminCheck = await pool.query('SELECT id_usuario_admin FROM administrador WHERE id_usuario_admin = $1', [user.id_usuario]);
      if (adminCheck.rows.length > 0) {
        userRole = 'administrador';
      } else {
        // Verificar si es empleado
        const empleadoCheck = await pool.query('SELECT id_usuario_empleado FROM empleado WHERE id_usuario_empleado = $1', [user.id_usuario]);
      ...
      const token = jwt.sign(
        ...
        { id: user.id_usuario, username: user.name_user, role: userRole }, 
